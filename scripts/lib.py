...
def current_control(L_x: float, L_y: float,
    ...
    """
    Masses and lengths:
    mb = 4.1kg, mu = 0.09kg, ml = 0.09kg
    rb = 0.15m, ru = 0.1375m, rl = 0.25m, g = 9.81m/s^2

    Kt is the torque constant. Currently pretty much randomly chosen
    """
    import math
    current_input = (
        (0.0916666666666667*L_x*math.cos(th_ul) + 0.0916666666666667*L_y*math.sin(th_ul) +
        0.00103125*dth_ll**2*math.sin(th_ll - th_ul) - 2.89120579329468e-19*dth_ll*dth_ul*math.sin(th_ll - th_ul) -
        0.000515625*dth_lr**2*math.sin(th_lr - th_ur) + 1.44560289664734e-19*dth_lr*dth_ur*math.sin(th_lr - th_ur) -
        0.12139875*math.sin(th_ul) + 0.060699375*math.sin(th_ur))/Kt,
        (-0.0458333333333333*L_x*math.cos(th_ul) - 0.0458333333333333*L_y*math.sin(th_ul) -
        0.000515625*dth_ll**2*math.sin(th_ll - th_ul) + 1.44560289664734e-19*dth_ll*dth_ul*math.sin(th_ll - th_ul) +
        0.00103125*dth_lr**2*math.sin(th_lr - th_ur) - 2.89120579329468e-19*dth_lr*dth_ur*math.sin(th_lr - th_ur) +
        0.060699375*math.sin(th_ul) - 0.12139875*math.sin(th_ur))/Kt,
    )
