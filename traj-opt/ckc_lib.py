...
from pyomo.environ import (
    ConcreteModel, RangeSet, Set, Param, Var, Constraint, Objective, value as pyovalue
)
...
#         return m.gamma[fe,cp] >= (m.foot_dx[fe,cp,pn] if pn == '+' else -m.foot_dx[fe,cp,pn])
    ...
    def friction_complementarity(m, fe):  # on friction cone OR no horizontal velocity
        if fe < m.fe[-1]:
    ...
    def slip_complementarity(m, fe, pn):  # horizontal force to right OR movement to right
        if fe < m.fe[-1]:
            α = sum(m.GRFx[fe,:,pn])
    ...
    if collocation == 'euler':
        m.interp_q  = Constraint(m.fe, m.cp, m.vars, rule=utils.implicit_euler(m.q,  m.dq))
        m.interp_dq = Constraint(m.fe, m.cp, m.vars, rule=utils.implicit_euler(m.dq, m.ddq))
    else:
...
def make_plots(m):
    ...
    q = utils.get_vals2(m, m.q, (m.vars,))
    plt.plot(q[:,0,:])
    plt.legend(list(m.vars))
    default_plot(plt, '$q$')
    
    # velocities
    dq = utils.get_vals2(m, m.dq, (m.vars,))
    plt.plot(dq[:,0,:])
    plt.legend(list(m.vars))
    default_plot(plt, '$\dot{q}$')
    
    # input torques
    Tc = utils.get_vals2(m, m.Tc, (m.Tc_set,))
    plt.step(range(m.fe[-1]), Tc)
    default_plot(plt, '$T_c$', '$Nm$\n(normalized to BW)')
    
    # ground reaction forces
    Lx = utils.get_vals2(m, m.GRFx, (m.posneg_set,))
    Ly = utils.get_vals2(m, m.GRFy, tuple())
    foot_height = utils.get_vals2(m, m.foot_height, tuple())
    ...
    # connection forces
    Fr = utils.get_vals2(m, m.Fr, (m.Fr_set,))
    plt.plot(Fr[:,0,:])
...
def get_joint_data(m, joint_position_funcs):
    ...
    q   = utils.get_vals2(m, m.q, (m.vars,))
    dq  = utils.get_vals2(m, m.dq, (m.vars,))
    ddq = utils.get_vals2(m, m.ddq, (m.vars,))
    Tc  = utils.get_vals2(m, m.Tc, (m.Tc_set,))
    Fr  = utils.get_vals2(m, m.Fr, (m.Fr_set,))
    Lx  = utils.get_vals2(m, m.GRFx, (m.posneg_set,))
    Ly  = utils.get_vals2(m, m.GRFy, tuple())
...
def make_animation(m, joint_position_funcs, h_m: float, body_width: float,
    ...
    from matplotlib import pyplot as plt
    import matplotlib.animation, sys
    ...
    # the ground
    ground = ax.plot([-5, 5], [0, 0], color='green', linewidth=2)[0]

    # Define the different elements in the animation
    body = plt.Rectangle([0, 0], body_width, body_width, color='red', angle=0)
    ax.add_patch(body)
    rods = [ax.plot([], [], color='grey', linewidth=2)[0]
            for i in range(4)]
    ...
    def animate(i):
        xb, yb = data['body'][i]
        ax.scatter([xb], [yb], color='gray', s=1)
